<script setup lang="ts">
    ...
    const handlerSubmit = async () => {
        ...
        await storeClient.handlerSubmitEdit(data)
    }

    watchEffect(async () => {
        if (!storeClient.isOpenEdit) {
            data.name = storeClient.client?.name!
            data.email = storeClient.client?.email!
            data.address = storeClient.client?.address!
            data.phone = storeClient.client?.phone!
            data.type = storeClient.client?.type!
            data.desType = storeClient.client?.type! === 'CORPORATE' ? 'JURIDICO' : 'NATURAL'
        }
    })
</script>

<template>
    <UModal v-model="storeClient.isOpenEdit" prevent-close>
        <UCard :ui="{ divide: 'divide-y divide-gray-100 dark:divide-gray-800' }" >
            <template #header class="overflow-y-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid"
                        @click="storeClient.isOpenEdit = false"
                    />
                </div>
            ...
            <template #footer>
                <div class="flex justify-end">
                    ...
                    <UButton color="red" @click="storeClient.isOpenEdit = false">Cancelar</UButton>
                </div>
            </template>
        </UCard>
    </UModal>
