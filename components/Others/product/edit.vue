<script setup lang="ts">
    ...
    const handlerSubmit = async () => {
        ...
        await storeProduct.handlerSubmitEdit(data)
    }

    watchEffect(async () => {
        if (!storeProduct.isOpenEdit) {
            ...
            data.stock = storeProduct.product?.stock!
        } else {
...
<template>
    <UModal v-model="storeProduct.isOpenEdit" prevent-close>
        <UCard :ui="{ divide: 'divide-y divide-gray-100 dark:divide-gray-800' }" >
            <template #header class="overflow-y-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid"
                        @click="storeProduct.isOpenEdit = false"
                    />
                </div>
            ...
            <template #footer>
                <div class="flex justify-end">
                    ...
                    <UButton color="red" @click="storeProduct.isOpenEdit = false">Cancelar</UButton>
                </div>
            </template>
        </UCard>
    </UModal>
